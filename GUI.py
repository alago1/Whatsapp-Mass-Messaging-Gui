# first version with all main functionalities working as intended
...
class LoginPage:
    ...
    def __init__(self, root):
        ...
        login_frame = tk.Frame(root, width=400, height=1000, bg='black')
        close_button_canvas = tk.Canvas(login_frame, width=30, height=30, bg='orange', highlightthickness=0)
        self._qr_code_canvas = tk.Canvas(login_frame, width=264, height=264, bg='white', highlightthickness=1)


        login_frame.grid(row=0, column=0, sticky='nsew')
        close_button_canvas.grid(row=0, column=0, sticky='ne')
        self._qr_code_canvas.grid(row=1, column=0, padx=50, pady=50)


        self._loading_text = self._qr_code_canvas.create_text(0, 0, anchor='nw', text='LOADING \nQR CODE...', font=('Courier', 20))
        close_button_canvas.create_text(5, 0, anchor='nw', text='X', font=('Courier', 25))
        ...
        close_button_canvas.bind('<Button-1>', lambda e: self.close_window(end_script=True))
        self._qr_code_thread = threading.Thread(target=self.update_qr_code, name='qr_code_thread')
        self._qr_code_thread.start()

    def center(self, root):
        """SETS LOGIN GUI's POSITION TO CENTER OF SCREEN"""
    ...
    def close_window(self, event=None, end_script=False):
        self._root.destroy()
        try:
            self._qr_code_thread.join()
        except:
            pass
        if end_script:
            sys.exit(0)
    ...
    def update_qr_code(self):
        """ FETCHES QR_CODE ON DRIVER, PROJECTS IT ON LOGIN PAGE
        SCHEDULES CHECK FOR LOGIN OR NEW QR_CODE FOR 5 SECONDS LATER"""
        update_time = 5*1000

        driver = get_driver()
        if driver is None:
            self._root.after(update_time, self.update_qr_code)
            return None
        ...
        try:
            filename = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'qr_code.png')
            qr_png = driver.get_qr(filename=filename)

            if hasattr(self, '_qr_code_img'):
                self._qr_code_canvas.delete(self._qr_code_img)
            else:
                self._qr_code_canvas.delete(self._loading_text)

            img = tk.PhotoImage(file=filename)
            self._qr_code_canvas.image = img
            self._qr_code_img = self._qr_code_canvas.create_image(0, 0, image=img, anchor='nw')

        except NoSuchElementException:
            print('did not find the qr code')
...
class MainWindow:
    ...
    def __init__(self, root):
        ...
        self._requested_chat_groupings = list()
    ...
    def update_search_entry_in_frame(self, page_name):
        frame = self._frames[page_name]
        frame.refresh_search_entry()

    def request_chat_grouping_creation(self, grouping):
        if hasattr(self, '_frames'):
            return self._frames['MessagingPage'].create_chat_grouping(grouping)
        self._requested_chat_groupings.append(grouping)
        return None
    ...
    def set_groupings_list(self, groupings_list):
        ...
        self.update_search_entry_in_frame('MessagingPage')
    ...
    def fetch_contact_list(self):
        ...
        for i in range(1, 25):
            self._contact_list.append(Contact('MyName'+str(i), 'MyCompanyName'+str(i), 'MyJob'+str(i), 'MyEmail'+str(i), 'MyPhone'+str(i), 'MyNotes'+str(i), id='id'))
    ...
    def fetch_groupings_list(self):
        ...
        for i in range(1, 25):
            ...
            for j in range(1, 5):
                grouping_members.append(Contact('MyName'+str(j), 'MyCompanyName'+str(j), 'MyJob'+str(j), 'MyEmail'+str(j), 'MyPhone'+str(j), 'MyNotes'+str(j), id='id'))
            self._groupings_list.append(Grouping('MyGrouping'+str(i), 'MyDescription'+str(i), grouping_members, self))
...
class ContactsPage(tk.Frame):
    ...
    def remove_contact(self, event=None):
        ...
        #updates groupings list and tree
        groupings_list = self._controller.get_groupings_list()
        for grouping in groupings_list:
            if member in grouping.get_members():
                grouping.remove_member(member)
        self._controller.update_search_entry_in_frame('GroupingsPage')

        #updates messaging chats

        self.delete_remove_contact_popup(force=True)

    def refresh_search_entry(self):
        self._search_entry.update_structure(self._contacts_list_tree)
...
class GroupingsPage(tk.Frame):
    ...
    def add_grouping(self, name, description, members):
        if not (name == '' and description == '' and members == list()):
            grouping = Grouping(name, description, members, self._controller)
            groupings_list = self._controller.get_groupings_list()
    ...
    def apply_edit_members(self):
        ...
        groupings_list[self._groupings_list_tree.index(item)] = Grouping(grouping.get_name(), grouping.get_description(), self._members_popup, self._controller)
    ...
    def refresh_search_entry(self):
        self._search_entry.update_structure(self._groupings_list_tree)
...
class MessagingPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self._search_groupings_entry = SearchEntry(messaging_contacts_frame, controller=self, borderwidth=1, font=('TimesNewRoman', 20), justify='center', idleText='Search Groupings')
        ...
        self._search_groupings_entry.grid(row=0, column=0, padx=10, pady=10, sticky='ew')
        ...
        # give to groupings that requested a chat_grouping
        for grouping in self._controller._requested_chat_groupings:
            grouping.set_chat_grouping(self.create_chat_grouping(grouping))
        del self._controller._requested_chat_groupings

        self.display_chat_groupings()
        self._search_groupings_entry.link(self._groupings_list_canvas, self._controller.get_groupings_list())
    ...
    def create_chat_grouping(self, grouping):
        chat_grouping = ChatGrouping(self._groupings_list_frame, controller=self, grouping=grouping)
        self._chat_groupings.append(chat_grouping)
        return chat_grouping
    ...
    def display_chat_groupings(self, subsection=None):
        ...
        if subsection is None:
            ...
            for grouping in groupings_list:
                self._displayed_chat_groupings.append(grouping.get_chat_grouping())
                grouping.get_chat_grouping().grid(sticky='nsew')

        else:
            # display only chat_groupings from groupings in the subsection
            for grouping in subsection:
                self._displayed_chat_groupings.append(grouping.get_chat_grouping())
                grouping.get_chat_grouping().grid(sticky='nsew')
    ...
    def refresh_search_entry(self):
        self._search_groupings_entry.update_structure(self._groupings_list_canvas, self._controller.get_groupings_list())
                ...
                for element in structure_elements:
                    ...
                    if element.get_members() is not None:
                        ...
                        for member in element.get_members():
                            ...
                            if member.get_name().lower().startswith(matchable_string):
                                # if match happens, add element to matched_items and look at next element
                                matched_items.append(element)
                                break
                # expose in the structure only the matched items
...
class ChatGrouping(tk.Canvas):
    ...
    def __init__(self, parent, controller, grouping, height=70, managing_page=None, *args, **kwargs):
        # Canvas Setup
        global chat_colors
        chat_colors = ['#0084FF', '#12CF13', '#FFC300', '#FB3C4C', '#D696BB']  # blue, green, yellow, red, purple

        tk.Canvas.__init__(self, parent, height=height, bg='white', *args, **kwargs)
        self._controller = controller

        self._grouping = grouping
        self._subtitle = self._grouping.get_description() #last message format :['', ''] # ['member_name', 'time_of_arrival']
        self._sent_messages = list()

        vocab = 'abcdefghijklmnopqrstuvwxyz0123456789'
        try:
            self._chat_color = chat_colors[vocab.index(grouping.get_name().lower()[0]) % len(chat_colors)] # chat color based on the first letter of the grouping name
        except:
            self._chat_color = chat_colors[0]

        if managing_page is None:
            self._managing_page = self._controller.create_managing_page(controller=self, bg='pink')
        else:
            self._managing_page = managing_page
        ...
        self._name_canvas.create_text(50, 15, text=self._grouping.get_name(), anchor='nw', font=('Courier', 18, 'bold')) # grouping name

        self._last_message_canvas = tk.Canvas(self, highlightthickness=0, height=18, bg='white') #height=10 or 20
        self._last_message_canvas.create_text(10, 0, text='Description: '.format(self._subtitle), anchor='nw')
    ...
    def get_name(self):
        return self._grouping.get_name()

    def get_description(self):
        return self._grouping.get_description()

    def get_members(self):
        return self._grouping.get_members()

    def get_grouping(self):
        return self._grouping

    def get_managing_page(self):
        return self._managing_page
...
class ManagingPage(tk.Frame):
    ...
    def __init__(self, parent, controller, chat_grouping=None, **kwargs):
        ...
        template_send_button = tk.Button(template_file_frame, text='Send', font=('Courier', 12), height=1, width=8, command=lambda:self.send_message(self._template_path_entry.get()))
        ...
        self._logs_messages_frame = tk.Frame(logs_canvas)
        ...
        self._logs_messages_frame.grid(row=0, column=0, sticky='nsew')
        ...
        logs_canvas.create_window(0, 0, anchor='nw', window=self._logs_messages_frame)
        logs_canvas.configure(yscrollcommand=logs_scrollbar.set)
        self.display_sent_messages()
    ...
    def send_message(self, template_path):
        """SENDS TEMPLATE MESSAGE TO ALL CONTACTS THAT ARE MEMBERS OF CHAT_GROUPING CONTROLLER"""
        driver = get_driver()
        with open(template_path, 'r') as message_file:
            message = message_file.read()
            self._controller.add_sent_message(message)
        for contact in self._controller.get_members():
            driver.send_message_to_id(contact.get_id(), message)
            print('sent message to ', contact.get_id())

    def display_sent_messages(self):
        messages = self._controller.get_sent_messages()
        for message, time in messages:
            message_canvas = tk.Canvas(self._logs_messages_frame)
            text_1 = message_canvas.create_text(5, 5, anchor='nw', text='Sent at {time}:'.format(time=time))
            bounds_1 = message_canvas.bbox(text_1)
            height_1 = bounds_1[3]-bounds_1[1]
            text_2 = message_canvas.create_text(10, 10+height_1, anchor='nw', text=message)
            bounds_2 = message_canvas.bbox(text_2)
            height_2 = bounds_2[3] - bounds_2[1]
            message_canvas.configure(height=10+height_1+height_2)
            message_canvas.grid(sticky='new', padx=5, pady=5)


class Contact():

    def __init__(self, name, company, job, email, phone, notes, driver=None, id=None):
        if phone is not None:
            if id is None:
                id = fetch_number_id(phone, driver=driver)
            if id is None:
                print('Could not locate id for {phone}'.format(phone))
    ...
    def __eq__(self, other):
        if self.get_name() == other.get_name() and self.get_company() == other.get_company() and self.get_job() == other.get_job() and self.get_email() == other.get_email() and self.get_phone() == other.get_phone() and self.get_notes() == other.get_notes() and self.get_id() == other.get_id():
            return True
        return False
...
class Grouping():
    ...
    def __init__(self, name, description, members, window_refer):

        self._name = name
        self._description = description
        self._members = sorted(members, key=lambda x: (len(x.get_name()), x.get_name()))
        self._chat_grouping = window_refer.request_chat_grouping_creation(self)
    ...
    def get_chat_grouping(self):
        return self._chat_grouping

    def set_chat_grouping(self, chat_grouping):
        self._chat_grouping = chat_grouping
    ...
    def __eq__(self, other):
        if self.get_name() == other.get_name() and self.get_description() == other.get_description() and self.get_members() == other.get_members():
            return True
        return False
...
def fetch_number_id(number, driver=None):
    driver = get_driver()

    id = None
    if driver.check_number_status(number + '@c.us').status == 200:
        id = number + '@c.us'
    elif driver.check_number_status(number + '@g.us').status == 200:
        id = number + '@g.us'

    return id


def initiate_driver(user_agent):
    global driver

    driver = WhatsAPIDriver(client='chrome', username='user', #profile=profile_path, #headless=True,
                            headless=True, chrome_options=['--user-agent='+user_agent], autoconnect=False)

    driver.connect()


def initiate_main_gui():
    global driver
    while True:
        time.sleep(0.1)
        if driver is not None and driver.is_logged_in():
            break
...
def initiate_login_gui():
    root = tk.Tk()
    login_gui = LoginPage(root)
    login_gui.center(root)
    root.mainloop()
...
if __name__ == '__main__':
    ...
    try:
        ...
        driver_thread = threading.Thread(target=initiate_driver, name='driver_thread', args=(user_agent, ))
        driver_thread.start()
